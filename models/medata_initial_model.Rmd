...
```{r}
print(modelo_logistico, pars = c("alpha", "beta"), probs = c(0.025, 0.975))
```
...
mcmc_acf(as.matrix(modelo_logistico, pars = c("alpha", "beta")))
mcmc_intervals(as.matrix(modelo_logistico, pars = "beta"))
mcmc_hist(as.matrix(modelo_logistico, pars = "beta"))
mcmc_trace(as.array(modelo_logistico), pars = c("alpha", "beta[1]", "beta[2]"))
```
...
posterior_samples <- extract(modelo_logistico)
...
# Matriz de confusión
confusion_matrix <- table(Real = muestra$ac_mental, Predicho = predicciones_binarias)
```
...
roc_obj <- roc(muestra$ac_mental, as.numeric(probs_predichas))
auc(roc_obj)
